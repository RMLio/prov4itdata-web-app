...
During the first step, the Web App obtains the credentials for authorized access to the services and the Solid pod.

In the second step, the RML.io toolset provides the necessary components for 1) consuming (protected) resources from a Web APIs, and 2) transformation to [RDF].

In the third step, the Web App finalizes the data transfer by storing the RDF, generated by the RMLMapper, to the Solid Pod.
